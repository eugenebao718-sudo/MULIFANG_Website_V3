...
export function LanguageSwitcher({ locale, mobile = false, label, onChangeComplete }: { locale: Locale; mobile?: boolean; label: string; onChangeComplete?: () => void }) {
  ...
  const change = (next: Locale) => {
    ...
    onChangeComplete?.();
    const target = `${switchLocale(pathname, next)}${window.location.search}${window.location.hash}`;
